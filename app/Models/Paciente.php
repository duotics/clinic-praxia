...
class Paciente
{
    ...
    function getPacList()
    {
        ...
        if ($this->TR > 0) {
            ...
            $this->pages->paginate();
            $this->RSp = $this->db->dbh->prepare($this->cadBus . $this->pages->limit);
            $this->RSp->execute();
    ...
    private function genCadSearchPac()
    {
        ...
        if ($this->termBus) {
            ...
            if ($cadBusN > 1) {
                $qry = sprintf(
                    "SELECT db_pacientes_nom.pac_cod AS code, CONCAT_WS(' ', db_pacientes_nom.pac_nom, db_pacientes_nom.pac_ape) AS value, 
                    MATCH (db_pacientes_nom.pac_nom, db_pacientes_nom.pac_ape) AGAINST (%s) AS Score 
            ...
            $qry = "SELECT db_pacientes.pac_cod AS code, CONCAT_WS(' ',db_pacientes.pac_nom, db_pacientes.pac_ape) AS value 
		FROM db_pacientes ORDER BY pac_cod DESC LIMIT {$limit}";
        }
        $this->cadBus = $qry;
    }
